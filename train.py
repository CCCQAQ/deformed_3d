    ...
    for iteration in range(1, opt.iterations + 1):
        ...
        if iteration < opt.warm_up:
            ...
            if not viewpoint_stack:
                viewpoint_stack = scene.getTrainSpatialCameras().copy()
        else:
            # pick a random temporal camera
            if not viewpoint_stack or iteration == opt.warm_up:
                viewpoint_stack = scene.getTrainTemporalCameras().copy()
        ...
        if iteration % 500 == 0:
            ...
            Image.fromarray(np.uint8(gt_img_vis * 255)).save(
                os.path.join(args.model_path, f"gt_{iteration}.png")
            )

            render_img_vis = image.permute(1, 2, 0).detach().cpu().numpy()
            render_img_vis = np.clip(render_img_vis, 0, 1)
            Image.fromarray(np.uint8(render_img_vis * 255)).save(
                os.path.join(args.model_path, f"render_{iteration}.png")
            )
            #####################################
        ...
        with torch.no_grad():
            ...
            if iteration in saving_iterations or iteration == opt.warm_up:
