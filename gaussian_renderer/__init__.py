...
def render_flow(viewpoint_camera, viewpoint_camera_prev, pc: GaussianModel, pipe, bg_color: torch.Tensor, d_xyz, d_rotation, d_scaling, d_xyz_prev, d_rotation_prev, d_scaling_prev, is_6dof=False,
           scaling_modifier=1.0, override_color=None):
    """
    Render the scene.
    ...
    if is_6dof:
        if torch.is_tensor(d_xyz_prev) is False:
            means3D_prev = pc.get_xyz
        else:
            means3D_prev = from_homogenous(
                torch.bmm(d_xyz_prev, to_homogenous(pc.get_xyz).unsqueeze(-1)).squeeze(-1))
    else:
        means3D_prev = pc.get_xyz + d_xyz_prev

    means2D , _  = point_cloud_to_image(means3D, viewpoint_camera)
    means2D_prev , _  = point_cloud_to_image(means3D_prev, viewpoint_camera_prev)
...
def render_surfel(viewpoint_camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, d_xyz, d_rotation, d_scaling, is_6dof=False,
           scaling_modifier=1.0, override_color=None):
    """
    Render the scene.
