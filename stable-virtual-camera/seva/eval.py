...
def get_wh_with_fixed_long_side(w, h, size):
    if size is None or size <= 0:
        return w, h
    if w >= h:
        new_w = size
...
def transform_img_and_K(
    ...
    if isinstance(size, (tuple, list)):
        ...
        # => if size is int, we rescale the image to fit the shortest side to size
        # => if size is None, no rescaling is applied
        W, H = get_wh_with_fixed_long_side(w, h, size)
    W, H = (
        math.floor(W / size_stride + 0.5) * size_stride,
        math.floor(H / size_stride + 0.5) * size_stride,
    )
...
def create_transforms_simple(save_path, img_paths, img_whs, c2ws, Ks, save_name="transforms.json"):
    ...
    with open(osp.join(save_path, save_name), "w") as of:
    ...
    for i, (img, K) in enumerate(zip(image_cond["img"], camera_cond["K"])):
        if isinstance(img, str) or img is None:
            ...
            if options.get("L_long", -1) == -1:
                ...
                downsample = 3
                assert options["L_long"] % F * 2**downsample == 0, (
                    "Long side of the image should be divisible by "
