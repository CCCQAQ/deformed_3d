...
class SceneInfo(NamedTuple):
    ...
    ply_path: str
    train_temporal_cameras: list
    train_spatial_cameras: list
...
#######################################################################
def readCamerasFromOpenglTransforms(path, temporal_data_path_list, transformsfile, max_time):
    # cam_infos = []
    cam_spatial_infos = []
    cam_temporal_infos = []
    ...
    for idx, frame in enumerate(contents["frames"]):
        ...
        if int(image_name.split(".")[0])==0:
            for temporal_idx, temporal_data_path in enumerate(temporal_data_path_list):
                temporal_data_name = Path(temporal_data_path).stem
                temporal_image = Image.open(temporal_data_path)
...
def readStableCameraInfos(path):
    temporal_data_paths = os.path.join(path, "temporal")
    print("temporal_data_path",temporal_data_paths)
    temporal_data_paths_list = sorted(glob(temporal_data_paths+"/*"))
    max_time = len(temporal_data_paths_list)
    print("Reading Training Transforms")
    ...
    scene_info = SceneInfo(point_cloud=pcd,
                           ...
                           nerf_normalization=nerf_normalization,
                           ply_path=ply_path,
                           train_temporal_cameras=train_temporal_cam_infos,
                           train_spatial_cameras=train_spatial_cam_infos)
    return scene_info
...
sceneLoadTypeCallbacks = {
    ...
    "stable_camera": readStableCameraInfos,
}
